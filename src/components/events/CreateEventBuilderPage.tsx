...
const POPULAR_TIMEZONES = [
  ...
  {
    label: 'Brasilia Standard Time - São Paulo',
    offset: 'GMT-03:00',
...
function DropPortal({
  ...
  useEffect(() => {
    const update = () => {
      ...
      left = Math.max(
        gutter,
        Math.min(left, window.innerWidth - nextWidth - gutter)
      );
...
function CalendarPopup({
  ...
  const days = (() => {
    ...
    for (let i = firstDow - 1; i >= 0; i--)
      arr.push({
        day: prevLast - i,
        iso: localIso(new Date(y, m - 1, prevLast - i)),
        cur: false,
      });
    for (let i = 1; i <= inMonth; i++)
      arr.push({ day: i, iso: localIso(new Date(y, m, i)), cur: true });
    while (arr.length < 42)
      arr.push({
        day: arr.length - firstDow - inMonth + 1,
        iso: localIso(new Date(y, m + 1, arr.length - firstDow - inMonth + 1)),
        cur: false,
      });
    return arr;
  })();
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            {([-1, 1] as const).map((offset) => (
              <button
                key={offset}
                type="button"
                onClick={() => setView(new Date(y, m + offset, 1))}
                style={{
                  background: 'none',
                  border: 'none',
                  cursor: 'pointer',
...
function TimePicker({
  ...
  useEffect(() => {
    ...
    if (listRef.current && idx >= 0) listRef.current.scrollTop = idx * 44 - 88;
  }, [value]);
...
function TimezonePicker({
  ...
  value: string;
  onChange: (v: (typeof POPULAR_TIMEZONES)[0]) => void;
  onClose: () => void;
  C: ReturnType<typeof makeColors>;
}) {
  ...
  const filtered = q
    ? POPULAR_TIMEZONES.filter(
        (z) =>
          z.label.toLowerCase().includes(q.toLowerCase()) ||
          z.offset.includes(q)
      )
    : POPULAR_TIMEZONES;
  return (
    ...
    >
      <div
        style={{
          padding: '12px 14px',
          borderBottom: `1px solid ${C.tzBorder}`,
        }}
      >
        <input
          value={q}
        ...
        {filtered.map((tz, i) => (
          <div
            ...
            style={{
              ...
              padding: '10px 14px',
              cursor: 'pointer',
              background: value === tz.tz ? C.inputBg : 'transparent',
              transition: 'background 0.1s',
            }}
            onMouseEnter={(e) => {
              if (value !== tz.tz)
...
function VisibilityPicker({
  ...
  const opts = [
    {
      id: 'published' as EventStatus,
      icon: <PiGlobe size={16} />,
      label: 'Public',
      desc: 'Shown on your calendar and eligible to be featured.',
    },
    {
      id: 'private' as EventStatus,
      icon: <PiSparkle size={16} />,
      label: 'Private',
      ...
      {opts.map((o, i) => (
        <div
          ...
          style={{
            ...
            padding: '14px 16px',
            cursor: 'pointer',
            background: value === o.id ? C.inputBg : 'transparent',
            borderBottom:
              i < opts.length - 1 ? `1px solid ${C.tzBorder}` : 'none',
            transition: 'background 0.1s',
          }}
          onMouseEnter={(e) => {
            if (value !== o.id)
        ...
        >
          ...
          {value === o.id && (
            <PiCheck size={16} style={{ marginTop: 2, color: C.accent }} />
          )}
        </div>
      ))}
    </div>
    ...
    minimal: (
      <div
        style={{
          width: '100%',
          height: '100%',
          background: 'linear-gradient(145deg,#f3eff8,#ede8f4)',
          display: 'flex',
    ...
    quantum: (
      <div
        style={{
          width: '100%',
          height: '100%',
          background: 'linear-gradient(135deg,#0d0b1e,#1e1050,#2d1b69)',
          overflow: 'hidden',
          position: 'relative',
          display: 'flex',
          alignItems: 'center',
        ...
        {[0, 1, 2].map((i) => (
          <div
            ...
            style={{
              ...
              width: `${60 + i * 20}%`,
              height: '1px',
              background: `linear-gradient(90deg,transparent,${i === 0 ? '#a78bfa' : i === 1 ? '#818cf8' : '#e879f9'},transparent)`,
              opacity: 0.6 + i * 0.15,
    ...
    emoji: (
      <div
        style={{
          width: '100%',
          height: '100%',
          background: 'linear-gradient(135deg,#181528,#241d40)',
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'center',
          flexWrap: 'wrap',
    ...
    confetti: (
      <div
        style={{
          width: '100%',
          height: '100%',
          background: 'linear-gradient(135deg,#1c0a38,#3b0764)',
          overflow: 'hidden',
          position: 'relative',
          display: 'flex',
          alignItems: 'center',
      ...
      >
        {['♥', '♥', '♥', '♥', '♥', '♥', '♥', '♥'].map((e, i) => (
          <span
            key={i}
            style={{
              color: ['#a78bfa', '#e879f9', '#c084fc'][i % 3],
              fontSize: 11 + (i % 3) * 2,
              opacity: 0.4 + i * 0.07,
    ...
    seasonal: (
      <div
        style={{
          width: '100%',
          height: '100%',
          background: 'linear-gradient(135deg,#fdf2f8,#fce7f3,#fdf4ff)',
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'center',
          flexWrap: 'wrap',
    ...
    >
      <div
        style={{
          ...
          boxShadow: selected
            ? `0 0 0 3px ${accent}30, 0 8px 24px rgba(0,0,0,0.18)`
...
export default function CreateEventBuilderPage() {
  ...
  const [timezone, setTimezone] = useState(
    POPULAR_TIMEZONES.find((z) => z.tz === 'Asia/Calcutta')!
  );
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const fb =
          cals.find((c) => c.id === searchParams?.get('calendar')) ||
  ...
  const setField = useCallback(
    <K extends keyof CreateEventForm>(k: K, v: CreateEventForm[K]) =>
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const createdSlug = String(res?.data?.slug || '');

      // Land on the Luma timeline so the newly created event opens in the detail panel right away.
      window.location.href = createdSlug
        ? `/luma/events?event=${encodeURIComponent(createdSlug)}&created=1`
        : '/luma/events';
    } catch (err: any) {
  ...
  const selectedCalendar =
    ownerCalendars.find((c) => c.id === selectedCalendarId) ||
    ownerCalendars[0];
  const toggle = (p: ActivePicker) =>
  ...
  return (
    ...
    >
      ...
      >
        <div
          className="create-builder-topbar"
          style={{
            display: 'flex',
            alignItems: 'center',
            gap: 20,
        ...
        >
          ...
          >
            GrowthLab{' '}
            <span style={{ fontWeight: 500, color: C.muted }}>Lite</span>
          </div>

          <div
            className="create-builder-divider"
            style={{
              width: 1,
              height: 18,
          ...
          {/* Nav Links */}
          <nav
            className="create-builder-topnav"
          ...
          {/* Controls & Avatar */}
          <div
            className="create-builder-controls"
            style={{
              display: 'flex',
              alignItems: 'center',
              gap: 14,
          ...
          >
            <div
              ...
              style={{
                ...
                padding: 4,
                borderRadius: '50%',
                background: isDark ? '#334155' : '#f9f9f9',
              }}
            >
              {isDark ? (
                <PiSun size={16} color={C.text} />
              ) : (
                <PiMoon size={16} color={C.text} />
              )}
            </div>
            <PiMagnifyingGlass
              size={18}
      ...
      {/* ── Builder ── */}
      <form
        className="create-event-builder-form"
      ...
      >
        {/* LEFT: Image + Theme */}
        <div
          className="create-event-builder-left"
        ...
        >
          <div
            style={{
              ...
              overflow: 'hidden',
              background: '#3b0764',
              boxShadow: `0 24px 60px rgba(0,0,0,${isDark ? 0.5 : 0.14})`,
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
            }}
          ...
          <div
            className="create-event-builder-theme-row"
                  ...
                  style={{
                    ...
                    background:
                      selectedTheme === 'minimal'
                        ...
                        : selectedTheme === 'warp'
                          ? '#060610'
                          : selectedTheme === 'quantum'
                            ? 'linear-gradient(135deg,#0d0b1e,#2d1b69)'
                            : selectedTheme === 'confetti'
                              ? 'linear-gradient(135deg,#1c0a38,#3b0764)'
        ...
        {/* RIGHT: Fields */}
        <div
          className="create-event-builder-right"
          style={{ display: 'flex', flexDirection: 'column', gap: 18 }}
        >
          {/* Header pills */}
          <div
            className="create-event-builder-pill-row"
            style={{
              display: 'flex',
              justifyContent: 'space-between',
              alignItems: 'center',
          ...
          >
            <div className="luma-pill">
              <span>🎃</span>
              <span>{selectedCalendar?.name || 'Personal Calendar'}</span>
              <PiCaretDown size={12} />
            </div>
            <div style={{ position: 'relative' }}>
              <div
                ...
                style={{
                  background: active === 'visibility' ? C.accent : C.pill,
                  color: active === 'visibility' ? '#fff' : C.text,
                  borderColor:
                    active === 'visibility' ? C.accent : C.chipBorder,
                }}
              >
                <PiGlobe size={14} />
                <span>
                  {form.status === 'published' ? 'Public' : 'Private'}
                </span>
                <PiCaretDown size={12} />
              </div>
              {active === 'visibility' && (
          ...
          {/* Title */}
          <input
            className="create-event-builder-title"
            ...
            style={{
              fontSize: '2.2rem',
              fontWeight: 700,
              color: form.title ? C.text : C.placeholder,
          ...
          >
            <div
              className="create-event-builder-datetime-row"
              style={{ display: 'grid', gridTemplateColumns: '1fr auto' }}
            >
              <div style={{ padding: '4px 0' }}>
                {/* START */}
                <div
                  className="create-event-builder-time-row"
                  style={{
                    display: 'flex',
                    alignItems: 'center',
                    gap: 10,
                ...
                >
                  ...
                  >
                    ...
                    <div
                      style={{
                        ...
                        left: 3.5,
                        height: 28,
                        borderLeft: `1.5px dashed ${C.light}`,
                      }}
                    />
                  </div>
                  <span
                  ...
                  <div
                    className="create-event-builder-time-chips"
                    style={{ display: 'flex', gap: 8, flex: 1 }}
                  >
                    <div ref={startDateRef} style={{ position: 'relative' }}>
                      <div
                        className={`date-chip${active === 'startDate' ? 'open' : ''}`}
                        onClick={(e) => {
                          e.stopPropagation();
                          setActive(
                    ...
                    <div ref={startTimeRef} style={{ position: 'relative' }}>
                      <div
                        className={`date-chip${active === 'startTime' ? 'open' : ''}`}
                        onClick={(e) => {
                          e.stopPropagation();
                          setActive(
                            active === 'startTime' ? null : 'startTime'
                          );
                        }}
                ...
                >
                  ...
                  >
                    <div ref={endDateRef} style={{ position: 'relative' }}>
                      <div
                        className={`date-chip${active === 'endDate' ? 'open' : ''}`}
                        onClick={(e) => {
                          e.stopPropagation();
                          setActive(active === 'endDate' ? null : 'endDate');
                        }}
                      >
                        {fmtDate(endDate)}
                      </div>
                    ...
                    <div ref={endTimeRef} style={{ position: 'relative' }}>
                      <div
                        className={`date-chip${active === 'endTime' ? 'open' : ''}`}
                        onClick={(e) => {
                          e.stopPropagation();
                          setActive(active === 'endTime' ? null : 'endTime');
                        }}
                        style={{ minWidth: 68, justifyContent: 'center' }}
                      >
                        {form.end_time}
              ...
              <div
                ref={timezoneRef}
                className="create-event-builder-timezone"
              ...
              >
                <div
                  ...
                  style={{
                    ...
                    gap: 3,
                    padding: '14px 16px',
                    borderLeft: `1px solid ${C.tzBorder}`,
                    background:
                      active === 'timezone' ? C.inputBg2 : 'transparent',
                    cursor: 'pointer',
                    height: '100%',
                    justifyContent: 'center',
                    minWidth: 100,
                ...
                >
                  ...
                  >
                    {timezone.label.split(' - ')[1] || timezone.label}
                  </div>
                </div>
                {active === 'timezone' && (
                  <TimezonePicker
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            >
              ...
              <div
                style={{ height: 1, background: C.divider, margin: '0 16px' }}
              />
              <div className="luma-option-row">
                <PiShieldCheck
                  size={18}
                ...
                <div
                  className={`luma-toggle${form.require_approval ? 'on' : ''}`}
                  onClick={() =>
                    setField('require_approval', !form.require_approval)
                  }
                />
              </div>
              <div
              ...
              <div className="luma-option-row">
                ...
                <EditOptionButton
                  value={form.limit_capacity ? form.max_seats : 'Unlimited'}
          ...
          <button
            ...
            style={{
              ...
              transition: 'background 0.15s',
              boxShadow: `0 8px 30px rgba(76,29,149,${isDark ? 0.4 : 0.25})`,
              marginTop: 4,
            }}
            onMouseEnter={(e) =>
      ...
      {showThemePanel && (
        <div
          className="create-builder-theme-panel"
          onClick={(e) => e.stopPropagation()}
          style={{
            position: 'fixed',
            bottom: 0,
            ...
            boxShadow: `0 -20px 70px rgba(0,0,0,${isDark ? 0.65 : 0.14})`,
            zIndex: 1000,
        ...
        >
          {/* Header */}
          <div
            className="create-builder-theme-panel-header"
            style={{
              display: 'flex',
              justifyContent: 'space-between',
              alignItems: 'center',
          ...
          >
            <div>
              ...
              <span style={{ fontSize: 12, color: C.muted, marginLeft: 10 }}>
                Pick a vibe for your event page
              </span>
            </div>
            <button
              type="button"
          ...
          <div
            className="create-builder-theme-grid"
            style={{
              display: 'flex',
              flexWrap: 'wrap',
              gap: 20,
      ...
      {showStripePopup && (
        ...
        >
          ...
          >
            ...
            >
              This calendar is not yet set up to accept payments.
            </p>
            <p
              style={{
                color: '#555',
            ...
            >
              We use{' '}
              <span style={{ color: '#0d9488', fontWeight: 600 }}>Stripe</span>{' '}
              to process payments. Connect or set up a Stripe account to start
      ...
      {showCapacityPopup && (
        ...
        >
          ...
          >
            ...
            >
              Close registration when reaching the capacity. Only approved
              guests count towards it.
            </p>
            ...
            <div
              style={{
                ...
                justifyContent: 'space-between',
                alignItems: 'center',
                marginBottom: form.limit_capacity ? 20 : 24,
              }}
            >
              ...
              <div
                className={`luma-toggle${form.limit_capacity ? 'on' : ''}`}
                onClick={() => setField('limit_capacity', !form.limit_capacity)}
                style={form.limit_capacity ? { background: '#22c55e' } : {}}
              />
            </div>
            ...
            {form.limit_capacity && (
              ...
              >
                ...
                <input
                  ...
                  onChange={(e) =>
                    setField('max_seats', parseInt(e.target.value) || 0)
                  }
                  style={{
                    width: 80,
            ...
            {form.limit_capacity && (
              <>
                ...
                >
                  ...
                  <div
                    className={`luma-toggle${form.waitlist_enabled ? 'on' : ''}`}
                    onClick={() =>
                      setField('waitlist_enabled', !form.waitlist_enabled)
                    }
                  />
                </div>
              </>
